...
// Run: ./node_modules/.bin/ts-node scripts/freezeAccount.ts
...
async function main(): Promise<void> {
    ...
    const accountId = await readLine("accountId to freeze");
    if (!accountId) {
        return;
    }
    ...
    if (account.frozen) {
        log.info("Account with accountId", accountId, "is already frozen");
    }
    ...
    const updateReq = dynameh.requestBuilder.buildUpdateInputFromActions(tableSchema, account, {
        attribute: "frozen",
        action: "put",
        value: `Frozen by script on ${new Date().toISOString()}`
    });
