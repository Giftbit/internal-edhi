...
describe("/v2/account", () => {
    ...
    describe("requireMfa (tests are interdependent)", () => {
        ...
        it("requires existing users without mfa enabled to do so before switching to the account", async () => {
            ...
            chai.assert.equal(switchAccountNoMfaResp.body.user.mfa, null);
            ...
            chai.assert.equal(switchAccountWithMfaResp.body.user.mfa, "totp");
        ...
        it("requires existing users without mfa enabled to do so before logging into the account", async () => {
            ...
            chai.assert.equal(loginNoMfaResp.body.user.mfa, null);
            ...
            chai.assert.equal(loginWithMfaCompleteResp.body.user.mfa, "totp");
